...
export default class JavaScriptFile {
	constructor(source, sourceType = "script") {
		this.source = source;
		this.sourceType = sourceType;
	}

	toClassModelArray() {
		let ast = Acorn.parse(this.source, { sourceType: this.sourceType });
		let classModels = [];
			...
			},
			{
				...
				MethodDefinition: (node, state, continueFn) => {
					if (node.kind === "constructor") {
						...
						let methodModel = new MethodModel(node.key.name, this.source.substring(node.start, node.end));
				...
				AssignmentExpression: (node, state, continueFn) => {
					if (state.currentlyParsing.type === "constructor") {
						...
						if (node.type === "AssignmentExpression" && node.left.object.type === "ThisExpression") {
							state.classModel.variables.push(new VariableModel(node.left.property.name, this.source.substring(node.start, node.end)));
						}
	...
	_addVariableReferenceToMethodLike(node, state, classModelSection) {
